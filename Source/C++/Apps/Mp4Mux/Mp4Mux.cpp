...
+---------------------------------------------------------------------*/
static AP4_Result
SetDoviLevel(AP4_UI32 video_width, AP4_UI32 video_height, double frame_rate, AP4_UI32& dv_level)
{
    ...
    for(;;) {
        ...
        if (AP4_SUCCEEDED(result)) {
            ...
            if (frame.m_LittleEndian) {
                AP4_ByteSwap(sample_data.UseData(), frame.m_Info.m_FrameSize);
            }
                 ...
                 SampleFileStorage&    sample_storage)
{
    ...
    SetDoviLevel(video_width, video_height, frame_rate, dv_level);
    // setup the video the sample descripton
