...
class ByteStream(object):
    ...
    def read_string(self, length):
        ...
        res = lb4.AP4_ByteStream_ReadString(self.stream, p, c_ulong(length+1))
        if res != 0:
            raise StreamException(res)
        return p.value
    ...
    def write_ui32(self, value):
        res = lb4.AP4_ByteStream_WriteUI32(self.stream, c_ulong(value))
        if res != 0:
            raise StreamException(res)
                                              
    def write_ui24(self, value):
        res = lb4.AP4_ByteStream_WriteUI24(self.stream, c_ulong(value))
        if res != 0:
            raise StreamException(res)
    ...
    def seek(self, position):
        res = lb4.AP4_ByteStream_Seek(self.stream, c_ulong(position))
        if res != 0:
            raise StreamException(res)
...
class MemoryByteStream(ByteStream):
    ...
    def FromBuffer(buffer):
        """Factory method"""
        buffer_size = c_ulong(len(buffer))
    ...
    def __init__(self, size=0, stream=None):
        if stream is None:
            stream = lb4.AP4_ByteStream_Create(c_ulong(size))
