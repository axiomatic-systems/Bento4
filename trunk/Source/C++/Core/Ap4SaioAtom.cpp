...
AP4_SaioAtom::AddEntry(AP4_UI64 offset)
{
    ...
    SetSize(AP4_FULL_ATOM_HEADER_SIZE+4+m_Entries.ItemCount()*(m_Version==0?4:8));
    return AP4_SUCCESS;
}
    ...
    AP4_Atom(AP4_ATOM_TYPE_SAIO, size, version, flags)
{
    ...
    if (size < AP4_FULL_ATOM_HEADER_SIZE+4+entry_count*(m_Version==0?4:8)) {
        return;
    }
...
{
    AP4_Result result = stream.WriteUI32(m_Entries.ItemCount());
